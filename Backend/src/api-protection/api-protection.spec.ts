...
describe('API Protection (API_PROTECTION_README.md)', () => {
  it('loads ddosGuard middleware', () => {
    const mod = require('../../middleware/ddosGuard');
  ...
  it('loads throttle middleware', () => {
    const mod = require('../../middleware/throttle');
  ...
  it('loads version middleware', () => {
    const mod = require('../../middleware/version');
    expect(typeof mod.versionMiddleware).toBe('function');
    const mw = mod.versionMiddleware({ defaultVersion: 'v2', supportedVersions: ['v1','v2'] });
    expect(typeof mw).toBe('function');
  });

  it('loads backwardCompat middleware', () => {
    const mod = require('../../middleware/backwardCompat');
